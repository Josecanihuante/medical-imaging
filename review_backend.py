...
def main():
    ...
    try:
        with open("app/services/inference.py", "r") as f:
            content = f.read()
    ...
    try:
        with open("app/services/inference.py", "r") as f:
            ...
            if match:
                ...
                print("   [FAIL] No se encontro parametro tta_steps")
    ...
    try:
        with open("app/routers/analysis.py", "r") as f:
            content = f.read()
            ...
            print(f"   [{status}] Validacion de MIME types")
            results.append(("Validacion Content-Type", has_validation))
    except Exception as e:
        print(f"   [FAIL] Error checking MIME validation: {e}")
        results.append(("Validacion Content-Type", False))
    
    # 7. Limite de tamaño (10MB) validado en router
    print("7. Limite de tamaño (10MB) validado en router")
    try:
        with open("app/routers/analysis.py", "r") as f:
            content = f.read()
            has_size_limit = "MAX_FILE_SIZE" in content and "10 * 1024 * 1024" in content
            status = "PASS" if has_size_limit else "FAIL"
            print(f"   [{status}] Limite de tamaño 10MB")
            results.append(("Limite tamaño 10MB", has_size_limit))
    except Exception as e:
        print(f"   [FAIL] Error checking size limit: {e}")
        results.append(("Limite tamaño 10MB", False))
    ...
    try:
        with open("app/services/gradcam.py", "r") as f:
            content = f.read()
    ...
    try:
        with open("../README.md", "r") as f:
            content = f.read()
            disclaimer_found = "Disclaimer clinico" in content
        status = "PASS" if disclaimer_found else "FAIL"
        print(f"   [{status}] Mencionado en README")
        results.append(("Disclaimer clinico documentado", disclaimer_found))
    except Exception as e:
        print(f"   [FAIL] Error checking README: {e}")
        results.append(("Disclaimer clinico documentado", False))
